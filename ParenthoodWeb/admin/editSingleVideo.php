...
if (isset($_GET['s_edit'])) {
    $id = $_GET['s_edit'];
    $update = true;
    $record = mysqli_query($db, "SELECT * FROM singlevideo WHERE s_id=$id");
    if ($record->num_rows > 0) {
        while ($n = $record->fetch_assoc()) {
            $id   = $n['s_id'];
            $title = $n['title'];
            $videoLink = $n['videoLink'];
            $categoryType = $n['categoryType'];
        }
    }
}
?>
...
<head>
    ...
    <title>ADMIN PANEL - EDIT SINGLE VIDEO</title>
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid" style="width: 1300px;"><br />
                    <!-- Page Heading -->
                    <center>
                        <h1 class="h3 mb-1 text-gray-800">UPDATE SINGLE VIDEO</h1>
                    </center>

                    <form method="post" action="viewSingleVideo.php" style="width: 60%">
                        <div class="row">
                            ...
                            <div class="col-15">
                                ...
                                <input type="text" name="videoLink" placeholder="Enter Youtube Video Link Here..." class="form-control" value="<?php echo $videoLink ?>" required><br>
                            </div>

                        </div>
                        <input type="hidden" name="s_id" value="<?php echo $id; ?>">
                        <?php if ($update == true) : ?>
                            <button class="btn btn-primary" type="submit" name="s_update">update</button>
                        <?php else : ?>
                            <button class="btn" type="submit" name="singlevideo">Update</button>
                        <?php endif ?>
